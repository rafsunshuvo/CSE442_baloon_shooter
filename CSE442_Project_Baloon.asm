...
arrow_status db 0d                          ;0d = arrow ready to go else not 
arrow_limit dw  88d                         ;150d
...
dw '                             |               |',0ah,0dh          ;new line
dw '                             |---------------|',0ah,0dh          ;new line
dw '                             | ^   Score   ^ |',0ah,0dh          ;new line
dw '                             |_______________|',0ah,0dh          ;new line
dw ' ',0ah,0dh 
dw ' ',0ah,0dh
dw ' ',0ah,0dh
dw ' ',0ah,0dh
...
mov ax, 0B800h                             ;Graphical Repretation
...
main_loop:                                 ;update logic and display everything
    ...
    inside_loop:                           ;checking every thing
        ...
        render_loon:                       ;draw baloon            ;render=to transmit to another 
            ...
            mov cl, 15d                    ;8-bit data registers: AH, AL, BH, BL, CH, CL, DH, and DL.
            mov ch, 1101b
        ...
        render_arrow:                      ;render/draw arrow
                                           
                                          
            mov cl, ' '                    ;8-bit data registers: AH, AL, BH, BL, CH, CL, DH, and DL.
            mov ch, 1111b
        ...
        inside_loop2:
            ...
            mov cl, 125d                  ;draw player who shoots bullet 
            mov ch, 1100b
...
key_pressed:                              ;input handling section
    mov ah,0
    int 16h
...
game_menu:
    ...
    int 21h
                                           ;wait for input
    input2:
        mov ah,1                           ;al and ah are the 8-bit, "char" size registers.  al is the low 8 bits, ah is the high 8 bits.
        ...
        jne input2                         ;jump not equal
        call clear_screen
        
        lea bx,state_buf                   ;display score
        call show_score 
        lea dx,state_buf                   ;Load effective address(lea)=Used to load the address of the operand into the provided register. 
        mov ah,09h
        int 21h
...
proc show_score
    ...
    mov [bx],   9d              ;branch & exchange instruction set
    mov [bx+1], 9d
    mov [bx+2], 9d
    mov [bx+3], 9d
    mov [bx+4], 'H'
